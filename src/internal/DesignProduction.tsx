...
export default function DesignProduction({ state, today, open }: {
    ...
    <div className="design-task-list">
      {shown.map(piece => {
        ...
        return <article className="design-task" key={piece.id} aria-label={piece.title}>
          ...
          <div className="design-task-actions"><span className="design-stage">Para producción</span>
            ...
            {APP_MODE!=='demo'&&<button className="button secondary" onClick={()=>open(piece.id,'delivery')}>Entrega</button>}<button className="button primary" onClick={() => open(piece.id)}>Abrir pieza<ArrowRight size={16}/></button></div>
          </div>
        </article>;
      })}
      {!shown.length && <div className="design-empty"><h2>{actionable.length ? 'No hay pendientes con estos filtros.' : 'No hay piezas listas para Diseño.'}</h2><p>{actionable.length ? 'Podés cambiar el cliente o la búsqueda.' : 'Las piezas aparecerán acá cuando Marketing las pase a producción.'}</p>{(clientId !== 'all' || search) && <button className="button secondary" onClick={() => { setClientId('all'); setSearch(''); }}>Quitar filtros</button>}</div>}
    </div>
