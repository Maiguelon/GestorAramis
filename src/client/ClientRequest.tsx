...
export default function ClientRequest({ token }: { token: string }) {
  ...
  return <div className="client-app">
    ...
    <main className={`client-request-main${material ? ' is-material' : ''}`}>
      <div className="client-request-heading"><div className="client-request-eyebrow"><span className="client-section-kicker">{review ? 'UN PASO MÁS CERCA DE PUBLICAR' : 'CREEMOS EL PRÓXIMO CONTENIDO'}</span><span className="client-request-version">{review ? `Versión ${review.version}` : 'Pedido de material'}</span></div><h1>{piece.title}</h1><p>{review ? 'Preparamos este contenido para tu marca. Miralo con calma y contanos qué te parece.' : 'Con tu material, el equipo puede seguir preparando esta pieza.'}</p><div className="client-request-meta"><span>{FORMAT_LABELS[piece.format]}</span><i /><span><CalendarDays size={14} />Publicación prevista: {dateLabel(piece.plannedDate)}</span></div></div>
