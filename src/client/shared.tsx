...
export function ClientHeader({ name, page }: { name: string; page: string }) {
  return <>
    ...
    <header className="client-header"><a href="/" className="client-brand" aria-label="Aramis, volver al equipo"><img src="/brand/aramis.svg" alt="Aramis" /></a><div className="client-header-right"><span>{name}</span><small>{page}</small></div></header>
  </>;
}

export function ClientFooter() {
  return <footer className="client-footer"><span>Aramis</span><span><ShieldCheck size={14} /> Enlace privado</span></footer>;
}

export function Unavailable({ error }: { error: string }) {
  return <div className="client-app"><ClientHeader name="Tu espacio" page="Acceso por enlace" /><main className="client-unavailable"><span className="client-section-kicker">NO PUDIMOS ABRIRLO</span><h1>Este enlace no está disponible.</h1><p>{error || 'Pedile al equipo de Aramis un enlace actualizado.'}</p><p>Si te llegó por WhatsApp, podés responder en ese mismo chat para pedir uno nuevo.</p><a className="client-btn client-btn-secondary" href="/"><ArrowLeft size={16} /> Volver a la demostración</a></main><ClientFooter /></div>;
}
